...
def main():
    if not os.path.exists('output/vrd'):
        pathlib.Path('output/vrd').mkdir(parents=True)
    pair_predicate_dict = defaultdict(set)
    triplet_frequency_dict = defaultdict(dict)
    with open('data/vrd/detections_train.json') as file_in:
        detections_train = json.load(file_in)
    with open('data/vrd/detections_val.json') as file_in:
        detections_valid = json.load(file_in)
    with open('data/vrd/new_annotations_train.json') as file_in:
        annotations_train = json.load(file_in)
    with open('data/vrd/new_annotations_val.json') as file_in:
    ...
    for image_id in train_vrd_dict:
        ...
        train_data[image_id] = {
            'bbox_list': bbox_list,
            'box_features': np.array([extract_box_features(box=box, num_classes=100) for box in bbox_list], dtype=np.float32),
            'vrd_list': vrd_list,
    ...
    dump_object_to_file(train_data, 'output/vrd/train_data.dat')
    dump_object_to_file(valid_data, 'output/vrd/valid_data.dat')
    dump_object_to_file(pair_predicate_dict, 'output/vrd/pair_predicate_dict.dat')
